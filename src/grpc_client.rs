...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let payment_req = tonic::Request::new(PaymentRequest {
        user_id: "user_123".to_string(),
        amount: 100.0,
    });
    let payment_resp = payment_client.process_payment(payment_req).await?;
    println!("RESPONSE={:?}", payment_resp.into_inner());

    let mut transaction_client = TransactionServiceClient::connect("http://[::1]:50051").await?;
    let transaction_req = tonic::Request::new(TransactionRequest {
        user_id: "user_123".to_string(),
    });
    let mut stream = transaction_client.get_transaction_history(transaction_req).await?.into_inner();
